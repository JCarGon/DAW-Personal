...
CREATE VIEW listado_pagos_clientes AS SELECT CONCAT(CLIENTE.NOMBRE_CONTACTO, " ", CLIENTE.APELLIDO_CONTACTO) AS "NONBRE Y APELLIDOS", 
CLIENTE.TELEFONO, CLIENTE.CIUDAD, CLIENTE.PAIS, PAGO.FECHA_PAGO AS "FECHA DEL PAGO", PAGO.TOTAL, PAGO.ID_TRANSACCION FROM CLIENTE NATURAL JOIN PAGO;

/*Todos incluyendo los que no hacen pedido*/
SELECT CONCAT(CLIENTE.NOMBRE_CONTACTO, " ", CLIENTE.APELLIDO_CONTACTO) AS "NONBRE Y APELLIDOS", 
CLIENTE.TELEFONO, CLIENTE.CIUDAD, CLIENTE.PAIS, PAGO.FECHA_PAGO AS "FECHA DEL PAGO", PAGO.TOTAL, PAGO.ID_TRANSACCION FROM CLIENTE LEFT JOIN PAGO 
...
CREATE VIEW listado_pedidos_clientes AS SELECT CONCAT(CLIENTE.NOMBRE_CONTACTO, " ", CLIENTE.APELLIDO_CONTACTO) AS "NONBRE Y APELLIDOS", 
CLIENTE.TELEFONO, CLIENTE.CIUDAD, CLIENTE.PAIS, PEDIDO.CODIGO_PEDIDO AS "CÓDIGO DEL PEDIDO", PEDIDO.FECHA_PEDIDO AS "FECHA DEL PEDIDO", 
PEDIDO.FECHA_ESPERADA AS "FECHA ESPERADA", PEDIDO.FECHA_ENTREGA AS "FECHA DE ENTREGA", SUM(CANTIDAD*DETALLE_PEDIDO.PRECIO_UNIDAD) AS "PRECIO TOTAL" FROM CLIENTE 
