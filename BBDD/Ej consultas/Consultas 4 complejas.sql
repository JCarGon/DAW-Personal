...
/*(CONSULTA MULTITABLA) Obtener un Listado con el nombre y apellidos del cliente, nombre de la población, código postal y nombre de la provincia de los clientes 
que habitan en Viveiro (POBLACION). Ordénalo por Nombre del Cliente*/
...
SELECT POBLACIONES.POBLACION, PROVINCIAS.PROVINCIA, MAX(CLIENTES.METROS) AS "METROS" FROM PROVINCIAS INNER JOIN POBLACIONES ON PROVINCIAS.CODIGOPROVINCIA = POBLACIONES.CODIGOPROVINCIA
